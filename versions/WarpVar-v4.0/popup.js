...
async function getAllVariables() {
    try {
        ...
        const results = await chrome.scripting.executeScript({
            ...
            func: () => {
                function findScratchVM() {
                    ...
                    if (appElement) {
                        const reactContainerKey = Object.keys(appElement).find(key => key.startsWith('__reactContainer$'));
                        if (reactContainerKey) {
                            try {
                                const rootFiber = appElement[reactContainerKey];
                                const AppFiber = rootFiber.child;
                                const store = AppFiber?.pendingProps?.store || AppFiber?.memoizedProps?.store;
                                if (store) {
                                    return store.getState().scratchGui.vm;
                                }
                            } catch (e) {
                                console.warn("Scratch Variable Controller: React detection failed", e);
                            }
                        }
                    }
                            ...
                            Object.values(target.variables).forEach(variable => {
                                ...
                                if (!variable.name.startsWith('_')) {
                                    variables.push({
                                        ...
                                        sprite: target.getName ? target.getName() : (target.isStage ? 'Stage' : 'Sprite'),
                                        id: variable.id
                                    });
                                }
                            });
                ...
                console.log(`Found ${variables.length} variables, ${Object.keys(locks).length} locked`);
                return { variables, locks };
            }
        });
        ...
        resultEl.textContent = "❌ Error: Cannot access Scratch page. Make sure you're on a Scratch or TurboWarp project.";
        return [];
    }
}

// Enhanced variable setting
async function setVariableValue(varName, newValue, foreverMode = false, intervalMs = 100) {
    try {
        const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
        if (!tab) {
        ...
        const results = await chrome.scripting.executeScript({
            ...
            func: (vName, vValue, isForeverMode, ms) => {
                            ...
                            if (target.variables) {
                                Object.values(target.variables).forEach(variable => {
                                    if (variable.name === vName) {
                                        // Type conversion
                                        if (variable.type === 'number' || variable.type === 'math_number') {
                                            actualValue = isNaN(parseFloat(vValue)) ? 0 : parseFloat(vValue);
                                        } else if (variable.type === 'boolean') {
                                            actualValue = Boolean(vValue && vValue !== 'false' && vValue !== '0');
                                        } else {
                                        ...
                                        // Update display
                                        if (target.blocks) {
                                            if (target.blocks._cache && typeof target.blocks._cache.variableChanged === 'function') {
                                                target.blocks._cache.variableChanged(vName, actualValue);
                                            }
                ...
                try {
                    ...
                    console.log(`Forever mode started for ${vName} = ${vValue} (every ${ms}ms)`);
                    return { success: true, message: `✅ Forever mode: "${vName}" = ${vValue} (${ms}ms)` };
                } catch (error) {
...
function renderVarOptions(vars) {
    ...
    if (vars.length === 0) {
        ...
        vars.forEach(v => {
            ...
            const valueDisplay = String(v.value).length > 20 ? String(v.value).substring(0, 20) + '...' : v.value;
...
function updateStats() {
    const lockedCount = Object.keys(lockedVars).length;
    statsEl.textContent = `Found ${allVars.length} variables | ${lockedCount} locked`;
}
...
cancelBtn.addEventListener("click", () => {
    ...
    resultEl.textContent = "Selection cleared";
});
    ...
    try {
        ...
        const result = await chrome.scripting.executeScript({
            ...
            func: () => {
                ...
                // Clear intervals
                if (window.__warpvar_intervals) {
                    Object.entries(window.__warpvar_intervals).forEach(([varName, intervalId]) => {
                        if (intervalId) {
                            clearInterval(intervalId);
                            clearedCount++;
                        }
                    });
                    window.__warpvar_intervals = {};
                }
                
                // Clear locks
                if (window.__warpvar_locks) {
                    clearedCount = Object.keys(window.__warpvar_locks).length;
                    window.__warpvar_locks = {};
                }
                
                return `Cleared ${clearedCount} locks`;
            }
        });
        
        resultEl.textContent = "✅ " + (result?.[0]?.result || "All locks cleared");
        setTimeout(refreshVariables, 300);
    } catch (error) {
        console.error("Error clearing locks:", error);
        resultEl.textContent = "❌ Error clearing locks: " + error.message;
    }
});
